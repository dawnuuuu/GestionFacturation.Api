...
using GestionFacturation.Api.Services;
...
namespace GestionFacturation.Api.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public AccountController(ApplicationDbContext dbContext, UserManager<ApplicationUser> manage,
        ...
        {
            var userRole = await _dbContext.UserRoles.FirstOrDefaultAsync(x => x.UserId == userId);
            if (userRole != null)
            {
        ...
        public async Task<IActionResult> Login(LoginModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                {
                    if (!await _manager.IsInRoleAsync(user, "Technicien") && !await _manager.IsInRoleAsync(user, "Admin"))
                    {
                        await _manager.AddToRoleAsync(user, "Technicien");
                    }
                }
            ...
            {
                return Unauthorized("As a result of the wrong registration, the account was temporarily withheld");
            }
        ...
        {
            var exist = await _dbContext.Users.AnyAsync(x => x.UserName == username);
            if (exist)
            {
                return StatusCode(StatusCodes.Status200OK);
            }
        ...
        {
            var exist = await _dbContext.Users.AnyAsync(x => x.Email == email);
            if (exist)
            {
                return StatusCode(StatusCodes.Status200OK);
            }
